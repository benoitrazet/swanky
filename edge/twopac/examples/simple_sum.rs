...
use fancy_garbling::{
    ...
    circuits::binary::BinaryAdditionNoCarry, util,
};
...
fn fancy_sum<F: FancyBinary>(
    ...
    // in a carry.
    let sum = BinaryAdditionNoCarry.execute(
        f,
        &(wire_inputs.garbler_wires, wire_inputs.evaluator_wires),
        channel,
    )?;
