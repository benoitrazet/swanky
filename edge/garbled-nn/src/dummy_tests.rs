...
use swanky_garbled_nn::util;
use swanky_garbled_nn::util::index_of_max;
...
pub fn arith_accuracy_test(
    ...
    for (img_num, img) in images.iter().enumerate() {
        ...
        if index_of_max(&res) != index_of_max(&labels[img_num]) {
            errors += 1;
        }
    }
...
pub fn boolean_accuracy_test(
    ...
    println!("* running circuit accuracy evaluation (binary)");
