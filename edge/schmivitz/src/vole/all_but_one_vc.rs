...
*/
use crate::vole::crypto_primitives::{Com, H1, IV, Key, Seed, h0};
...
fn tree(iv: IV, r: Key, depth: usize) -> (Keys, Vec<Seed>, Vec<Com>) {
    ...
    for d in 1..depth + 1 {
        ...
        for j in 0..n_previous_level {
            let mut prg = SwankyRng::from_seed(ks.get(d - 1, j).into());
            let (t1, t2) = prg.r#gen();
            ks.set(d, j * 2, t1);
            ks.set(d, j * 2 + 1, t2);
        }
...
#[cfg(test)]
mod test {
    use super::{IV, Key, Seed, commit, num_rec, open, reconstruct, verify};
