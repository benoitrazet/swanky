...
pub mod test {
    ...
    impl<F: FancyBinary> Circuit<F> for TestBinaryAdditionNoCarry {
        type Input = <BinaryAdditionNoCarry as Circuit<F>>::Input;
        type Output = <BinaryAdditionNoCarry as Circuit<F>>::Output;
        ...
        fn execute(
            ...
            BinaryAdditionNoCarry.execute(backend, inputs, channel)
        }
    }
    ...
    fn binary_addition() {
        ...
        let nbits = 64;
        let q = 1 << nbits;
        let c = TestBinaryAdditionNoCarry(nbits);
