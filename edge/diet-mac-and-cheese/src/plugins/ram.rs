...
#[derive(Clone, Copy, Debug)]
pub(crate) struct RamArithV0(pub Ram);

impl RamArithV0 {
    /// Create a new [`RamArithV0`] execution of `op` for a RAM over
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct RamBoolV0(pub Ram);

impl RamBoolV0 {
    /// Create a new [`RamBoolV0`] execution of `op` for a RAM over
...
pub(crate) enum RamVersion {
    /// A ram_bool_v0 execution.
    RamBoolV0(RamBoolV0),

    /// A ram_bool_v1 execution.
    RamBoolV1(RamBoolV1),

    /// A ram_arith_v0 execution.
    RamArithV0(RamArithV0),
...
impl RamVersion {
    ...
    pub fn type_id(&self) -> TypeId {
        match self {
            Self::RamBoolV0(RamBoolV0(Ram { field_id, .. }))
            | Self::RamBoolV1(RamBoolV1(Ram { field_id, .. }))
            | Self::RamArithV0(RamArithV0(Ram { field_id, .. }))
            | Self::RamArithV1(RamArithV1(Ram { field_id, .. })) => *field_id,
        }
    }
}

impl Plugin for RamBoolV0 {
    const NAME: &'static str = "ram_bool_v0";
    ...
    fn instantiate(
        ...
        match operation {
            "init" => {
                ...
                Ok(PluginExecution::Ram(RamVersion::RamBoolV0(RamBoolV0::new(
                    ram_output_type_id,
                    field_id,
                    addr_count as usize,
...
impl Plugin for RamArithV0 {
    const NAME: &'static str = "ram_arith_v0";
    ...
    fn instantiate(
        ...
        match operation {
            "init" => {
                ...
                Ok(PluginExecution::Ram(RamVersion::RamArithV0(
                    RamArithV0::new(ram_output_type_id, field_id, op),
                )))
            }
            "read" => {
                let op = RamOp::Read;
