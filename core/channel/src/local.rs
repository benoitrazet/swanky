...
fn tcp_socketpair() -> swanky_error::Result<(std::net::TcpStream, std::net::TcpStream)> {
    ...
    let server = std::net::TcpListener::bind("127.0.0.1:0")
        .wrap_err(ErrorKind::NetworkError, "Failed to bind to '127.0.0.1:0'.")?;
    let addr = server
        .local_addr()
        .wrap_err(ErrorKind::NetworkError, "Failed to get local_addr().")?;
    ...
    server.set_nonblocking(true).wrap_err(
        ...
        "Failed to set nonblocking on server",
    )?;
            ...
            Err(e) => {
                    ...
                    && e.kind() != std::io::ErrorKind::Interrupted
                {
                    return Err(e).wrap_err(
                        ErrorKind::NetworkError,
                        "Failed to accept() with a non-timeout-related error.",
                    );
                }
            }
        }
...
impl LocalSocket {
    ...
    pub fn pair() -> swanky_error::Result<(Self, Self)> {
        ...
        let (a, b) = std::os::unix::net::UnixStream::pair()
            .wrap_err(ErrorKind::NetworkError, "Failed to construct LocalSocket.")?;
        #[cfg(not(unix))]
        let (a, b) = tcp_socketpair()
            .wrap_err(ErrorKind::NetworkError, "Failed to construct LocalSocket.")?;
